...
<header class="fixed inset-x-0 top-0 z-50 bg-white shadow-md">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            <!-- Logo -->
            <a href="#" class="flex items-center gap-2 font-bold text-brand flex-shrink-0">
                @if(!empty($settings['logo_url']))
                    <img src="{{ $settings['logo_url'] }}" alt="{{ $settings['logo_text'] ?? 'Logo' }}" class="h-10 sm:h-12 w-auto object-contain"/>
                @else
                    <span class="text-sm sm:text-base">{{ $settings['logo_text'] ?? 'Erazehan' }}</span>
                @endif
            </a>
            ...
            <a href="{{ $settings['nav_cta_link'] ?? '#contact' }}"
               class="hidden md:inline-flex bg-brand text-white text-sm font-semibold px-5 py-2.5 rounded-full hover:bg-brand-dark transition-all duration-200">
                {{ $settings['nav_cta_text'] ?? 'Free Consultation' }}
            </a>

            <!-- Mobile Menu Button -->
            <button id="mobileMenuBtn" class="md:hidden p-2 text-gray-700 hover:text-brand transition-colors" type="button">
    ...
    <!-- Mobile Menu -->
    <div id="mobileMenuPanel" class="hidden md:hidden bg-white border-t border-gray-200">
        <nav class="px-4 py-4 space-y-2">
            @foreach($navItems as $item)
            <a href="{{ $item->url }}" class="block py-2 px-3 text-gray-700 hover:text-brand hover:bg-gray-50 rounded transition-colors">
                {{ $item->label }}
            </a>
            @endforeach
            <a href="{{ $settings['nav_cta_link'] ?? '#contact' }}"
               class="block w-full mt-4 bg-brand text-white text-center py-2.5 px-3 rounded-lg font-semibold hover:bg-brand-dark transition-all">
                {{ $settings['nav_cta_text'] ?? 'Free Consultation' }}
            </a>
        </nav>
    </div>
